export default function About() {
    return (
      <div className="about bg-cover bg-center bg-no-repeat w-full h-full" style={{backgroundImage: "url('/img/about.jpg')",}}>
            <br />
            <center>
        <div className="about-box block bg-[rgba(0,0,0,0.7)] w-[50%] h-full mx-[10px] my-[100px] line-height-[40px] border-2 border-black border-dotted rounded-[25px] text-white">
          <h1 className="about-h1 capitalize text-[30px] m-2">About Us</h1>
          <p>
            At [Brand Name], we believe that furniture is more than just functional — it&apos;s an essential part of creating spaces that inspire, comfort, and bring people together. Founded with a passion for exceptional design and craftsmanship, we are committed to producing furniture that not only enhances the beauty of your home but also improves your everyday living experience.
          </p>
          <p>
            We specialize in a wide range of designs, from contemporary and minimalist to classic and timeless. Each piece in our collection is carefully crafted using high-quality, sustainably sourced materials, ensuring that our furniture is as durable as it is beautiful. Whether you&apos;re looking for a statement piece for your living room, a cozy chair to relax in, or a sturdy desk for your home office, our products are designed to offer both style and function.
          </p>
          <p>
            We understand that buying furniture is an investment in your home and your lifestyle. That&apos;s why we take pride in the details, from the craftsmanship that goes into every joint and finish to the thoughtful design choices that make each piece versatile and long-lasting. We work closely with artisans and designers to ensure that every item meets our high standards of quality and design integrity.
          </p>
          <p>
            At [Brand Name], we&apos;re not just creating furniture — we&apos;re helping you create spaces where memories are made, relationships are nurtured, and comfort is found. Whether you&apos;re outfitting a single room or transforming your entire home, we&apos;re here to support you every step of the way.
          </p>
          <p>
            Thank you for choosing [Brand Name]. We look forward to being part of your journey to a more beautiful, functional home.
          </p>
        </div>
        </center>
        <br />
      </div>
    );
  }
  